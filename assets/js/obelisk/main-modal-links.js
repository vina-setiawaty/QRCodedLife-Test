/* ==========================================================================
   main-modal-links.js — Prototype 2 bootstrap.

   Same scene as Prototype 1, but only the 3 QR codes in `modalRoutes` are
   live (the rest are dimmed + inert). Opening a modal pushes the URL to
   .../modal-N/ (no reload); closing it pops back to the base URL; loading
   .../modal-N/ directly (a real folder — GitHub Pages has no server-side
   rewrites) opens that modal on load.
   ========================================================================== */
...
import { modalRoutes, invert } from "./routes.js";

const qrIdForSlug = invert(modalRoutes);
...
function basePath() {
  return dirPath().replace(/modal-\d\/?$/, "");
}

function slugFromPath() {
  const m = dirPath().match(/modal-\d(?=\/?$)/);
  return m ? m[0] : null;
}

function syncFromUrl() {
  const slug = slugFromPath();
  const qrId = slug && qrIdForSlug[slug];
  if (qrId) openModal(qrId);
  else closeModal();
}
...
function init() {
  ...
  obelisk.qrTargets.forEach((panel) =>
    setPanelEnabled(panel, Boolean(modalRoutes[panel.userData.qrId])),
  );
...
function pickPanel() {
  ...
  for (const hit of hits) {
    ...
    return null; // nearest solid thing is not a live panel -> it occludes
  }
  return null;
}
...
function onPointerUp(e) {
  ...
  if (panel) {
    ...
    openModal(qrId);
    history.pushState({ qrId }, "", basePath() + modalRoutes[qrId] + "/");
  }
}
